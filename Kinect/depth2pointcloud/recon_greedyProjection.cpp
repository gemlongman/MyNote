...
{
    std::string pointFileName=std::string(argv[1]);
    std::string meshFileName=std::string(argv[2]);
    // 点云模型读入,此处读入为PCD格式点云文件.数据类型为PointXYZ.
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);
    if (pcl::io::loadPCDFile<pcl::PointXYZ>(pointFileName, *cloud) == -1)
    {
    ...
    pcl::io::saveVTKFile(meshFileName+".vtk", triangles); 
    pcl::io::saveOBJFile(meshFileName+".obj", triangles); 
